...
| 对比项 | 桌面版 | 网页版 |
| --- | --- | --- |
| 运行方式 | `Electron` 本地运行 | `Express + Vite` 部署后通过浏览器访问 |
| 输出方式 | 直接保存到你选择的本地目录 | 单文件下载 `.glb`，批量下载 `.zip` |
| 输出目录 | 支持手动选择 | 不预选本地目录 |
| 输出位置操作 | 支持“打开输出位置” | 不提供本地路径操作 |
| 批量输入 | 支持多文件和文件夹 | 支持多文件和文件夹上传 |
| 适用场景 | 本机处理、交付文件、桌面工具使用 | 服务器部署、局域网共享、浏览器访问 |
