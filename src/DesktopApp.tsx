...
function getParentDirectoryPath(targetPath: string) {
  const lastSeparatorIndex = Math.max(targetPath.lastIndexOf('/'), targetPath.lastIndexOf('\\'))
  return lastSeparatorIndex >= 0 ? targetPath.slice(0, lastSeparatorIndex) : targetPath
}
...
function App() {
  ...
  const [saveNotice, setSaveNotice] = useState<{ tone: 'error' | 'success'; text: string } | null>(null)
  ...
  const selectedItems = useMemo<SelectedItem[]>(
    () =>
      selectedDirectory
        ? selectedDirectory.entries.map((entry) => ({
  ...
  async function openFolderPicker() {
    if (isDesktop && window.desktopBridge?.pickInputDirectory) {
      const outcome = await window.desktopBridge.pickInputDirectory()
      if (outcome.canceled) {
        return
      }
      ...
      if (outcome.error) {
        ...
        setSelectedDirectory(null)
        resetProcessingState()
        setError({ error: outcome.error })
        return
      }

      const entries = outcome.entries ?? []
      if (!outcome.directoryPath || entries.length === 0) {
        setSelectedFiles([])
        setSelectedDirectory(null)
        resetProcessingState()
        setError({ error: '选中的文件夹中没有可处理的 .glb 文件。' })
        return
      }

      setSelectedFiles([])
      setSelectedDirectory({
        directoryPath: outcome.directoryPath,
        entries,
      })
      resetProcessingState()
      return
  ...
  async function pickOutputDirectory() {
    if (!window.desktopBridge || isPickingOutputDir) {
      return
    }
    ...
    try {
      const outcome = await window.desktopBridge.pickOutputDirectory()
      if (outcome.canceled) {
        return
      }
      ...
      if (outcome.directoryPath) {
        setOutputDirectory(outcome.directoryPath)
      }
    } finally {
  ...
  async function revealPath(targetPath: string) {
    if (!window.desktopBridge || !targetPath || isRevealingPath) {
      return
    }
    ...
    try {
      const outcome = await window.desktopBridge.revealPath(targetPath)
      if (outcome.error) {
        setSaveNotice({
          tone: 'error',
          text: outcome.error,
  ...
  async function handleSubmit() {
    ...
    const endpoint = isBatchMode ? '/api/optimize/batch' : '/api/optimize'

    formData.append('mode', 'visual-lossy')
    formData.append('outputDir', outputDirectory)
    ...
    if (isBatchMode) {
      if (selectedDirectory) {
        formData.append('directoryPath', selectedDirectory.directoryPath)
      } else {
  ...
  const resultPath = result?.outputFile.filePath ?? ''
  const singleOutputDirectory = resultPath ? getParentDirectoryPath(resultPath) : outputDirectory
  const batchOutputDirectory = batchResult?.outputDirectory ?? outputDirectory
  const resultSummary = result
    ? `最终压缩比例为 ${formatPercent(result.savingsPercent)}，结果已保存到 ${singleOutputDirectory || '输出目录'}。`
    : batchResult
      ? `整批最终压缩比例为 ${formatPercent(batchResult.totalSavingsPercent)}，结果已保存到 ${batchOutputDirectory || '输出目录'}。`
      : '处理完成后，结果会直接保存到你选择的输出目录。'
  ...
  function renderOutputSettingsSection(displayPath: string, revealTarget: string) {
    return (
      <div className="result-note output-settings">
          ...
          <div className="output-settings__body">
            ...
            <div className="output-settings__row">
              <div className="output-settings__label">输出目录</div>
              <div className="output-settings__control output-settings__control--directory">
                <div className={`output-settings__path ${displayPath ? '' : 'output-settings__path--empty'}`}>
                  {displayPath || '请先选择输出目录。'}
                </div>
                {isDesktop ? (
                  <button type="button" className="toolbar-button" onClick={() => void pickOutputDirectory()} disabled={isPickingOutputDir}>
                    <FolderIcon />
                    <span>{isPickingOutputDir ? '选择中...' : '选择目录'}</span>
                  </button>
                ) : (
                  <span className="output-settings__desktop-note">仅桌面版可选</span>
                )}
              </div>
            </div>
            ...
            <button
              type="button"
              className="action-button action-button--block"
              onClick={() => void revealPath(revealTarget)}
              disabled={!revealTarget || !isDesktop || isRevealingPath}
            >
              <FolderIcon className="action-icon" />
              <span>{isRevealingPath ? '打开中...' : '打开输出位置'}</span>
            </button>
          </div>
      </div>
    )
    ...
    <main className="app-shell">
      <section className="app-frame">
        <header className="app-header">
          <div className="brand-block">
            ...
            <div className="brand-copy">
              ...
              <p>本地运行，不上传文件。适合 GLB 的交付压缩、批量整理与展示前优化。</p>
            </div>
          </div>
              ...
              <div className="upload-meta">
                ...
                <p>
                  {selectedFileCount > 0
                    ...
                    : isBatchMode
                      ? '选择多个文件或整个文件夹后，程序会把其中所有 GLB 直接压缩并保存到你指定的输出目录。'
                      : '选择一个 GLB 文件后，程序会自动尝试多档压缩方案，并把结果直接保存到你指定的输出目录。'}
                </p>
              </div>
          ...
          <article className="panel-card pipeline-panel">
            ...
            <div className="panel-card__body pipeline-panel__body">
              ...
              <p className="panel-note">
                点击开始后，流程会按步骤推进；结果会直接保存到你选择的输出目录。
              </p>
            </div>
          </article>
            ...
            <div className="panel-card__body result-panel__body">
              ...
              {result ? (
                <div ref={resultPanelBodyRef} className="result-panel__scroll">
                  <>
                    {renderOutputSettingsSection(singleOutputDirectory, resultPath || singleOutputDirectory)}
                ...
                <div className="batch-results-layout">
                  ...
                  <section className="batch-settings-section">
                    {renderOutputSettingsSection(batchOutputDirectory, batchOutputDirectory)}
                  </section>
                </div>
              ) : (
                <div ref={resultPanelBodyRef} className="result-panel__scroll">
                  {renderOutputSettingsSection(outputDirectory, outputDirectory)}
                </div>
              )}
            </div>
          </article>
        ...
        <footer className="app-footer">
          <span>本地运行</span>
          <span>版本 1.0.0</span>
          <span>隐私：所有处理都在本地完成，不上传任何文件。</span>
        </footer>
              ...
              <div className="guide-dialog__body">
                <section className="guide-section guide-section--compact">
                  ...
                  <ul>
                    ...
                    <li>批量压缩：可选多个 GLB，也可直接选整个文件夹。</li>
                    <li>批量选择文件夹后，会递归处理其中全部 .glb 文件。</li>
                  </ul>
                </section>
                ...
                <section className="guide-section guide-section--compact">
                  <h3>结果会怎样保存</h3>
                  <ul>
                    <li>先选输出目录，处理完成后结果会直接保存进去。</li>
                    <li>批量模式会按真实完成数量显示进度，不是假动画。</li>
                    <li>这是视觉压缩，目标是尽量小且尽量保真，不是严格无损。</li>
                  </ul>
                </section>
              </div>
            </section>
